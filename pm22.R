...
m2x <- function(qval, new.dat){
  # splitting data by domestic/international
  i.dat <- new.dat[which(new.dat$ISI_STATUS == "ISI"),]
  d.dat <- new.dat[which(new.dat$ISI_STATUS == "Domestic"),]
  # Column names to read data
  cnames <- colnames(new.dat)
  rc_list <- rc_list.get(qval, new.dat)
  rc_list <- rc_eval("m2x",rc_list)
